...
def coherent(alphas, cis, rl, nr, normalized=True):
    ...
    for i in range(n):
        ai = alphas[i]
        ci = cis[i]
        w = w + np.abs(ci) ** 2 * xwig.coherent(ai, ai, rl, nr)
        norm = norm + np.abs(ci)**2
        for j in range(i+1, n):
            aj = alphas[j]
            cj = cis[j]
            w = w + 2*np.real(ci * np.conj(cj) * xwig.coherent(ai, aj, rl, nr))
...
def gaussian(alpha_list, xi_list, amplitude_list, rl, nr, normalized=True):
    ...
    for i in range(n):
        ...
        xi = xi_list[i]
        ci = amplitude_list[i]
        w = w + np.abs(ci) ** 2 * xwig.gaussian(ai, ai, xi, xi, rl, nr)
        norm = norm + np.abs(ci)**2
        for j in range(i+1, n):
            aj = alpha_list[j]
            xj = xi_list[j]
            cj = amplitude_list[j]
            w = w + 2*np.real(ci * np.conj(cj) * xwig.gaussian(ai, aj, xi, xj, rl, nr))
...
def gaussian_fock(alpha_list, xi_list, n_list, amplitude_list, rl, nr, normalized=True):
    ...
    for i in range(n):
        ...
        ni = n_list[i]
        ci = amplitude_list[i]
        w = w + np.abs(ci) ** 2 * xwig.gaussian_fock(ai, ai, xi, xi, ni, ni, rl, nr)
        norm = norm + np.abs(ci)**2
        for j in range(i+1, n):
            aj = alpha_list[j]
            xj = xi_list[j]
            nj = n_list[j]
            cj = amplitude_list[j]
            w = w + 2*np.real(ci * np.conj(cj) * xwig.gaussian_fock(ai, aj, xi, xj, ni, nj, rl, nr))
