...
def fock_wigner(n, rl, nr):
    ...
    w = fock_radial(n, np.sqrt(mx ** 2 + my ** 2))
    return w


def fock_cross_wigner_xp(i, j, x, p):
...
def fock_cross_wigner(i, j, rl, nr):
    x = np.linspace(-rl/2, rl/2, nr)
    mx, mp = np.meshgrid(x, x, indexing='ij')
    wij = fock_cross_wigner_xp(i, j, mx, mp)
    return wij


def fock_cross_wigner_set(N, rl, nr):
    wijset = np.zeros([N, N, nr, nr], dtype=complex)
    for i in range(N):
        wijset[i][i] = fock_wigner(i, rl, nr)
        for j in range(i + 1, N):
            wij = fock_cross_wigner(i, j, rl, nr)
            wijset[i][j] = wij
            wijset[j][i] = np.conj(wij)
    return wijset


def displaced_fock_cross_wigner_xp(m, n, alpha, beta, x, p):
...
def displaced_fock_cross_wigner(m, n, alpha, beta, rl, nr):
    # Return the Wigner function of the operator D(alpha)|m><n|D(-beta)
    x = np.linspace(-rl/2, rl/2, nr)
    mx, mp = np.meshgrid(x, x, indexing='ij')
    wij = displaced_fock_cross_wigner_xp(m, n, alpha, beta, mx, mp)
    return wij


def coherent_cross_wigner_xp(alpha, beta, x, p):
...
def coherent_cross_wigner(a, b, alpha):
...
def squeezed_coherent_cross_wigner(alpha, beta, rl, nr, sq=1):
    x = np.linspace(-rl/2, rl/2, nr)
    mx, mp = np.meshgrid(x, x, indexing='ij')
    wij = coherent_cross_wigner_xp(alpha, beta, sq*mx, mp/sq)
    return wij


def squeezed_coherent_superposition_wigner(alphas, cis, rl, nr, sq=1, normalized=True):
    w = np.zeros([nr, nr])
    n = len(alphas)
    norm = 0
    for i in range(n):
        ai = alphas[i]
        ci = cis[i]
        w = w + np.abs(ci)**2*squeezed_coherent_cross_wigner(ai, ai, rl, nr, sq)
        norm = norm + np.abs(ci)**2
        for j in range(i+1, n):
            aj = alphas[j]
            cj = cis[j]
            w = w + 2*np.real(ci*np.conj(cj)*squeezed_coherent_cross_wigner(ai, aj, rl, nr, sq))
...
def particle_in_a_box_wigner(a, rl, nr):
    x = np.linspace(-rl/2, rl/2, nr)
    mx, mp = np.meshgrid(x, x, indexing='ij')
...
def gaussian_wigner(rl, nr, alpha=0, covmat=np.eye(2)/2):
...
def psi_cross_wigner(fi, fj, rl, nr):
...
def density_to_wigner(rho, rl, nr, isherm=True):
    ...
    if isherm:
        for i in range(n):
            rii = rho[i][i]
            if rii != 0:
                w = w + rii * fock_cross_wigner(i, i, rl, nr)
            for j in range(i+1, n):
                rij = rho[i][j]
                if rij != 0:
                    rho_wij = rho[i][j] * fock_cross_wigner(i, j, rl, nr)
        ...
        for i in range(n):
            for j in range(n):
                rij = rho[i][j]
                if rij != 0:
                    w = w + rho[i][j] * fock_cross_wigner(i, j, rl, nr)
    return w
